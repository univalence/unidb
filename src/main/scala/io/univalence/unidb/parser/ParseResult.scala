...
enum ParseResult[I, +A](val input: Input[I]) {
  ...
  def mapFailure(f: (String, Input[I]) => String): ParseResult[I, A] =
    this match {
  ...
  def mapError(f: (String, Input[I]) => String): ParseResult[I, A] =
    this match {
      case Success(_, _)        => this
      case Failure(_, _)        => this
      case Error(reason, input) => Error(f(reason, input), input)
    }
  ...
  def commit: ParseResult[I, A] =
    this match {
      case Success(_, input) =>
        if (input.hasNext)
          Error("failed while parsing the whole input", input)
        else
          this
